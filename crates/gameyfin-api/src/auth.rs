...
/// How a request is authenticated against the server.
#[async_trait]
pub trait AuthStrategy: Send + Sync + std::fmt::Debug {
    ...
    /// Called after the server rejected a request with 401/403.
    ///
    /// Returns `true` if state was refreshed and the call is worth retrying once. The
    /// default is `false`, a strategy with nothing to refresh should not cause retries.
...
/// Bearer-token auth against the device-token endpoints (server plan, PR A).
#[derive(Debug, Clone)]
...
impl DeviceTokenAuth {
    ...
    /// Replace the stored token, e.g. after re-pairing the device.
...
/// The CSRF token to send, and under which header name.
...
/// Spring's default CSRF cookie and header names.
const SPRING_CSRF_COOKIE: &str = "XSRF-TOKEN";
const SPRING_CSRF_HEADER: &str = "X-XSRF-TOKEN";
/// Vaadin's own CSRF cookie and header names, used when Spring's are absent.
const VAADIN_CSRF_COOKIE: &str = "csrfToken";
const VAADIN_CSRF_HEADER: &str = "X-CSRF-Token";

/// CSRF details scraped from the served index page.
///
/// Spring's cookie repository puts the token in a cookie and only the header *name* in the
/// page, while its session repository puts the token in the page too, so the header name
/// and the token are looked up independently rather than as a pair.
...
impl CookieSessionAuth {
    ...
    /// Fetch and parse the index page for CSRF material, caching the result.
    async fn page_csrf(&self) -> PageCsrf {
        ...
        // Deliberately `/login`, not `/`.
        //
        // On a server with SSO configured, an unauthenticated GET to `/` hits the
        // authentication entry point and is redirected to `/oauth2/authorization/oidc`,
        // which starts a *new* authorization request server-side and overwrites the one
        // the user is part-way through, so their real callback then fails with
        // `/login?error`. `/login` is permitAll, carries the same CSRF meta tags, and
        // triggers nothing.
        //
        // Redirects are not followed for the same reason: whatever a redirect leads to
        // here is something we did not intend to request.
        let req = self
            .http
        ...
        // Cached even when empty, so a server that exposes nothing is not re-fetched per call.
        *self.page_csrf.write().await = Some(info.clone());
        info
    }

    /// Resolve the CSRF token to send, following Hilla's own resolution order:
    /// a Spring token wins, otherwise the Vaadin token is used.
...
impl AuthStrategy for CookieSessionAuth {
    async fn apply(&self, req: RequestBuilder) -> ApiResult<RequestBuilder> {
        ...
        let mut req = req.header(reqwest::header::COOKIE, Self::cookie_header(&cookies));
...
/// Extract CSRF material from a served index page.
fn parse_page_csrf(body: &str) -> PageCsrf {
    // Compiled per call rather than kept in a lazy static: this runs at most once per
    // session reset, so the clarity is worth more than the microseconds.
    let header_re =
