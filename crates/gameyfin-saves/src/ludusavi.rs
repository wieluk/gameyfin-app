...
impl Ludusavi {
    ...
    /// Whether Ludusavi may refresh the database itself: a daily check in the middle of a
    /// backup, whose failure is made non-fatal rather than breaking that backup.
    ...
    /// Refreshes the database of save locations. `force` is what a button press means:
    /// Ludusavi otherwise skips any check made in the last day.
