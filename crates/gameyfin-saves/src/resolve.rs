...
/// What is known about a game when trying to identify it.
...
/// Resolve a game to a manifest title, escalating only as far as needed.
pub async fn resolve(lud: &Ludusavi, identity: &GameIdentity) -> SaveResult<TitleMatch> {
    // 1. Store IDs are exact. This is why exposing `originalIds` to users matters.
...
mod tests {
    ...
    async fn high_fuzzy_score_is_still_never_accepted_unattended() {
        ...
        // GameVault would have taken this silently. We ask.
