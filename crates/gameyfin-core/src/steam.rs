...
//! # The format
//!
//! A tag byte, a NUL-terminated key, then a value whose shape the tag decides:
//!
//! | Tag  | Meaning                                    |
//! |------|--------------------------------------------|
//! | 0x00 | nested map, terminated by its own 0x08      |
...
/// One node of a binary VDF document.
///
/// `Map` keeps insertion order, because Steam indexes shortcuts by their *position*
/// (the keys are "0", "1", "2", …) and reordering them silently reassigns entries.
...
/// Tag bytes, named rather than spelled inline at each use.
mod tag {
...
/// Parse a binary VDF document.
///
/// The top level is a map even though it has no opening tag of its own, which is why this
/// reads entries until the input runs out rather than until a terminator.
...
impl Shortcut {
    /// The id Steam derives for a non-Steam game.
    ///
    /// CRC-32 of the exe and name concatenated, with the top bit set. Computed rather than
    /// left to Steam because it is also the name of the artwork files in `grid/`, and
    /// because a stable id is what lets a second run recognise its own entry.
    ...
    fn to_value(&self, existing: Option<&Value>) -> Value {
        // Start from the entry already in the file, so fields Steam maintains itself
        // (LastPlayTime, per-game controller config, artwork ids) are not reset every
        // time the app writes.
...
/// Add or update one shortcut in a parsed `shortcuts.vdf` document.
///
/// Matching is by app id, which is derived from the exe and the name, so running this
/// twice for the same game updates the entry rather than adding a duplicate.
...
/// Names of the shortcuts already in the document, for showing what is registered.
...
/// The `shortcuts` list, created if the document does not have one yet.
...
/// Where Steam might be installed, relative to the user's home directory.
///
/// The Flatpak path is listed because a user running Steam as a Flatpak has their
/// `userdata` inside that sandbox, and the native path will not exist at all.
...
/// Write a document back, keeping one backup of what was there before.
///
/// The backup is the point. This file is the only record of every non-Steam game the user
/// has, from every launcher, and a bug here would be both silent and unrecoverable.
...
/// CRC-32 (IEEE, reflected), the variant Steam uses to derive a shortcut's id.
///
/// Table-driven and computed on first use rather than pulled in as a dependency: it is
/// twenty lines, and this is the only place in the app that needs a CRC.
struct Crc32(u32);
...
/// The reflected polynomial table, built at compile time.
...
mod tests {
    ...
    #[test]
    fn an_app_id_always_has_its_top_bit_set() {
        // Steam stores it signed, so this is what makes it read as negative there.
    ...
    #[test]
    fn the_app_id_is_stable_for_the_same_game() {
    ...
    fn keys_written_by_other_tools_are_preserved() {
        // The whole reason for parsing into a generic tree: Heroic and Lutris write here
        // too, and losing their entries would be an unrecoverable bug.
