//! Copying saves between stores. Nothing is deleted from the source, so a half-finished
...
#[derive(Debug, Default, Clone, PartialEq, Eq, Serialize, ts_rs::TS)]
...
/// Copy every game's saves from `source` to `destination`: only the newest version of each
/// unless `all_versions` is set.
    ...
    F: FnMut(usize, usize),
{
    ...
    for (index, game_id) in games.iter().copied().enumerate() {
        ...
        for version in chosen {
            ...
            // The id came from the source store, so it is not a name to build a path from.
            let archive = scratch.join(format!(
                "{game_id}-{}.zip",
                crate::save_store::safe_id(&version.id)
            ));
