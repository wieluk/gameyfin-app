...
pub struct AppState {
    ...
    /// When a game's first-start save prompt was answered, so the launch it starts does not
    /// check again what the answer just settled.
    answered_offers: Arc<Mutex<std::collections::HashMap<i64, Instant>>>,
...
impl AppState {
    ...
    pub fn answered_offers(&self) -> Arc<Mutex<std::collections::HashMap<i64, Instant>>> {
        self.answered_offers.clone()
    }
