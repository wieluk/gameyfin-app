//! Reading controllers on a dedicated thread, since `gilrs` polls and is not `Send`. Directions
//! repeat here, where the cadence is steady; what a button does is the frontend's call.
...
/// How long a direction is held before repeating: long enough that one deliberate press
...
/// The direction a stick position means. The larger axis wins, so a diagonal push moves
/// focus once rather than twice.
...
/// Starts the polling thread. A plain thread, not a task: `gilrs` is not `Send` and wants
/// polling rather than awaiting.
...
fn run(app: AppHandle, handle: Handle) {
    ...
    loop {
        ...
        // The first connected pad wins; several would need rules for conflicting directions.
