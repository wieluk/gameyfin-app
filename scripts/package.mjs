...
// Declared as sidecars in tauri.conf.json, so the build fails before it starts without
// them. Cheap when they are already there: the script checks the version and returns.
console.log("Checking the bundled helpers...");
execFileSync(process.execPath, [join(ROOT, "scripts", "fetch-sidecar.mjs")], {
  cwd: ROOT,
  stdio: "inherit",
});
...
// failed build would otherwise report success, listing stale installers as if fresh.
const startedAt = Date.now();
let buildFailed = false;

// Tauri fails on a missing signing key instead of skipping updater artifacts. CI sets the key.
...
try {
  ...
  // Tauri's output already reached the terminal, but a failure to start it prints nothing.
